---
title: underScore.js源码分析
date: 2016-09-21 21:53:43
tags: javascript underScore
---
## 前言

想做这件事也已经很久了，最初的想法是读jQuery的源码，但是最新版的jQuery已经有一万行左右了，实在是心有余而力不足，所有还是先从这个只有两千行左右的开始吧。
话说underScore.js还是很厚道的，它的开发版本的源码中自带注释。当然了，是英文的，那么就先从注释开始吧。
...
// 将this赋值给root,在浏览器中指代window，在服务器端指代exports
var root = this;
...
// 为了将来的代码压缩，把一些原型变量化。
// 因为Array.prototype.x = …是不能被压缩的，变成a.b.x编译器就不认识了，
//而变量化之后,ArrayProto则可以被压缩为a,a.x = …是没有问题的
var ArrayProto = Array.prototype, ObjProto = Object.prototype, FuncProto = Function.prototype;

// Create quick reference variables for speed access to core prototypes.
// 创建一些快速引用变量，一边可以快速使用一些核心的原型方法
var
...
// 声明`_` 对象（以安全引用的方式)_
// 类似使用jQuery时，把DOM对象jquery化一样
// 这里也是把对象underScore化的效果，包在wrapped里面
var _ = function(obj) {
...
// 如果是node.js这种服务器端的，则把`_`注入到exports里面
// 如果是浏览器，则把`_`注入root也就是this(window)中
...
// functions.
// 内部使用的函数，主要用于修改回调函数的调用对象（即this）
// 同时设置回调函数的形参顺序
// 如果回调函数不需要修改调用对象，即func.call(null,args);
// 为context传入undefined即可。
// 注意这里使用void 0 代表undefined 这是因为undefined在js中
// 并不属于保留字，而void 0 或 void(0)返回纯正的undefined
// argCount不传即为null针对的是调用回调函数的对象为数组或对象的时候
// 这时候回调函数的三个参数分别代表值，索引，和数组/对象本身 
// 传入对象本身是为了链式编程，每次执行方法都会把原始值原样返回
// argCount为1或2时没什么好说的，就是单值函数
// argCount为4主要针对reduce这种，和3个参数的相比的区别是
// 第一个arg传入的是初始值
...
//  模拟ES5中的create方法来创建对象
//  先调用内部函数isObject来判断传入的是不是obj，如果不是，则返回一个空对象
//  如果支持ES5原生Create方法，则直接调用并将结果返回
//  否则就把传入的对象放入Ctor这个空对象的原型中。
//  实例化Ctor之后并返回，同时把Ctor初始化为空
...
// 遍历类数组对象的每一个元素，或者遍历对象的每一个属性，执行传入的函数
// each和forEach相同
...
// 这里用length === +length来表示length为"number"类型
// +length可以把表示数字的字符串转为"number"型
// 注：
一元加操作符的操作与Number()函数相同。
// 也就是说这个if是为了判断那些有长度的，比如string类型或者function
// 对象这种没有length属性的就走到else去了
if (length === +length) {
for (i = 0; i < length; i++) {
iteratee(obj[i], i, obj);
}
} else {
// 如果是obj,则取出obj的所有属性去执行iteratee方法
var keys = _.keys(obj);
